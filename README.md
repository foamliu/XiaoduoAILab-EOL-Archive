...
### 模型开发
- **Xmodel系列**：开发了轻量级（1-2B）、强推理、具备Agent能力的大语言模型
- **多模态集成**：创新了统一视觉语言理解的新方法（XDP）
- **记忆系统**：创新了即插即用的记忆层，用于增强对话AI

### 基准评测与领域Agent落地
- **ECom-Bench**：建立了电商AI系统的全面评估框架
- **MindFlow**：在真实客服场景下验证领域Agent性能

### 应用研究
- **多模态**：探索了用于电商客服场景的多模态智能体架构形态
- **多语言**：通过与朱拉隆功大学（合作），验证了轻量级大模型的跨语言能力
- **效率验证**：将 Xmodel 系列模型在服务端（SGLang/vLLM）和客户端（Ollama）部署测试，验证了轻量级模型的低成本优势。
